...
list_train_val = open('./lists/lists_GaE_Gray/train.txt').readlines()
list_train = list_train_val[0:len(list_train_val)*6//7]
list_val = list_train_val[len(list_train_val)*6//7:]

list_test = open('./lists/lists_GaE_Gray/test.txt').readlines()

data_dir = '../data/GaE_Gray'
...
Grayscale_Model.compile(optimizer='adam', loss="binary_crossentropy", metrics=[iou_score])
#Grayscale_Model.summary()


checkpoint_path = "../Weights Grayscale Xception Test/cp.ckpt"
...
H = Grayscale_Model.fit_generator(generator_train(BS, train_aug),
...
with open('train_Xception_H.txt','wb') as file_H:
    pickle.dump(H.history, file_H)

#Grayscale_Pred = Grayscale_Model.predict_generator(generator_test(BS, test_aug), steps=len(list_test) // BS)

scores = Grayscale_Model.evaluate_generator(generator_test(BS, test_aug), workers=10, verbose=0)

with open('test_Xception_S.txt','wb') as file_S:
    pickle.dump(scores, file_S)

print(Grayscale_Model.metrics_names)
print('\n')
print(scores)
